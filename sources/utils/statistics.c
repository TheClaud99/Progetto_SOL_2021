...
void print_stats() {
    ...
    printf(
            ...
            (stats.max_size_reached / (double) 1000000), (config.max_memory_size / (double) 1000000), stats.max_file_number_reached,
            config.max_files, stats.n_replace_applied, stats.max_concurrent_connections, stats.blocked_connections
    );
    fflush(stdout);
}
