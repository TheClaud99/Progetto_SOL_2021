...
read -r -u 4 data avg_read avg_written n_read n_write n_lock n_open n_openlock n_unlock n_delete n_close max_size_reached max_file_number_reached n_replace_applied max_concurrent_connections blocked_connections
...
echo -e "$avg_read bytes\t$avg_written bytes\t$n_read\t$n_write\t$n_lock\t$n_open\t$n_openlock\t\t$n_unlock\t$n_delete\t$n_close"
echo ''
